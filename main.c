		...
		do
		{
			count++;
				do
				{
					...
					display_roll(*ptr_die_1, *ptr_die_2, *ptr_die_3, *ptr_die_4, *ptr_die_5);// Displays the roll in a friendly manner.
					use_roll(&use_roll_selection, roll_count);// determines if the player wants to use 

					if ((use_roll_selection == 1) || (roll_count == 3) ) //This part is for if the user chooses to pick a combination.
					{
						if (player == PLAYER1)
						{
